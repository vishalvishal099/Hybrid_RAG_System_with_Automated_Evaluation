"""
Generate sample fixed_urls.json file
This creates 200 diverse Wikipedia URLs for the project
"""
...
def generate_sample_fixed_urls():
    """
    Generate a sample set of 200 diverse Wikipedia URLs
    These cover various topics as required by the project
    """
    
    # Diverse Wikipedia articles across multiple domains
    urls = [
        # Science - Physics (20)
    ...
    # Verify we have 200 URLs
    assert len(urls) == 200, f"Expected 200 URLs, got {len(urls)}"
    ...
    data = {
        ...
        "description": "Fixed set of 200 diverse Wikipedia URLs covering multiple domains",
        "domains": {
            "Physics": 20,
            "Chemistry": 15,
            "Technology": 20,
...
def save_fixed_urls(output_path: str = "data/fixed_urls.json"):
    """Save fixed URLs to JSON file"""
    Path(output_path).parent.mkdir(parents=True, exist_ok=True)
    
    data = generate_sample_fixed_urls()
    
    with open(output_path, 'w') as f:
        json.dump(data, f, indent=2)
    
    print(f"✓ Saved {data['count']} fixed URLs to {output_path}")
    print(f"\nDomain distribution:")
